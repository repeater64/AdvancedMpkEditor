...
data class BarrelItem(
    ...
    override fun getTag(): NbtCompound {
        val (commands, info) = CommandsManager.generateCommands(AllCommandsSettings(fixedSlotsData))
        return buildNbtCompound {
            ...
            putNbtCompound("tag") {
                ...
                putNbtCompound("BlockEntityTag") {
                    ...
                    putNbtList("Items") {
                        add(HotbarFillingChest.getNbt(0))
                        add(InventoryFillingShulker.getNbt(1))
    ...
    companion object {
        fun fromTag(tag: NbtCompound) : BarrelItem {
            ...
            val commands = ((((items[2] as NbtCompound)["tag"]) as NbtCompound)["pages"] as NbtList<*>).map { it as NbtString }.map { it.value }
            val info = ((((items[6] as NbtCompound)["tag"]) as NbtCompound)["pages"] as NbtList<*>).map { it as NbtString }.map { it.value }

            val allCommandsSettings = CommandsManager.loadSettings(commands, info)

            return BarrelItem(name, practiceTypeOption, gamemodeOption, difficultyOption, allCommandsSettings.fixedSlotsData)
        }
    }
}